...
for nyr in range(1996,1999+1):
    ...
    for nmon in range(1,12+1):
        for nday in range(1,ndays[nmon-1]+1):
            file = vip_dir + str(nyr) + '/VIP-LD.HCo11T_avg_' + str(nyr)+'-'+str(nmon).zfill(2)+'-'+str(nday).zfill(2)+'T00:00:00.nc'
            pos_u_trans[nt], neg_u_trans[nt], delta_time[nt], date2[nt]  = get_transports(file)[:]
            nt+=1
print 'save to file'

# SAVE TO NETCDF file
file = 'vip_u_ALLm_transport_col'+str(vip1[0])+'_rows'+str(vip1[1])+'-'+str(vip2[1])+'.nc'
