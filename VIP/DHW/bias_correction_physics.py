...
import numpy as np
from lib_bias_correction import bias_correction as bc
...
for year in np.arange(first_year,last_year+1):
	...
	for month in np.arange(12):
		...
		for day in np.arange(days_in_month[month]):
			...
			nn_day = nn_day + 1
			for var in listvar:
				file_mom_in = var + mom_runname + datetag + '.nc'
				diff_file = dir_differences + 'monthly_difference_' + var + '_ESM2M-WOA13.nc'
