import netCDF4 as nc
import numpy as np
from matplotlib import pyplot as plt
import pyroms as py

# MONTH NAMES
title_val = ['JAN','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC']

VIP = py.grid.get_ROMS_grid('VIP')

nc_grid_file = '/t3/workdir/liz/VIP/VIP_grid/VIP_grd_high_res_bathy.nc'
fid = nc.Dataset(nc_grid_file, 'r')
mask_vals = fid.variables['mask_rho'[:]]
h = fid.variables['h'[:]]
s_rho = fid.variables['s_rho'[:]]

mask_grid = mask_vals[:]
depth = h[1:-1,1:-1]
s_rho = s_rho[:]

fid.close()

class LineBuilder:
    def __init__(self, line):
        self.line = line
        self.xs = list(line.get_xdata())
        self.ys = list(line.get_ydata())
        self.cid = line.figure.canvas.mpl_connect('button_press_event', self)

    def __call__(self, event):
        if event.inaxes!=self.line.axes: return
        self.xs.append(event.xdata)
        self.ys.append(event.ydata)
        self.line.set_data(self.xs, self.ys)
        self.line.figure.canvas.draw()
        if len(self.xs)==2:
           self.line.figure.canvas.mpl_disconnect(self.cid)


#fig = plt.figure()
#ax = fig.add_subplot(111)
#ax.set_title('click to build transect')
#line, = ax.plot([], [])  # empty line
#linebuilder = LineBuilder(line)
#plt.pcolor(mask_grid)

#plt.draw()
#plt.pause(6)
#plt.close()
# CALCULATE stuff

min_row = 90
max_row = 170
rows = np.arange(min_row,max_row+1)
col = 245

local_seg_width = (depth[rows,col]/len(s_rho)).reshape(1,len(rows))
dep_vec = (-1*(np.arange(len(s_rho))+.5)).reshape((len(s_rho),1))
local_dep_vals = np.multiply(local_seg_width,dep_vec)
X = np.tile(rows, (len(s_rho),1))

# TRANSECT LOCATION PLOT
plt.figure()
plt.pcolor(mask_grid)
plt.plot([col,col],[min_row,max_row],'g^-')
plt.tight_layout(pad=4, w_pad=5, h_pad=1.0)
plt.xlim([0,mask_grid.shape[1]])
plt.ylim([0,mask_grid.shape[0]])
print mask_grid.shape

# FIGURE CONFIGURATION
fig = plt.figure(num = 1, figsize = (25,7))
mngr = plt.get_current_fig_manager()
geom = mngr.window.geometry()
x,y,dx,dy = geom.getRect()
mngr.window.setGeometry(200,50,dx,dy)

n =0
for J in np.arange(len(title_val)):
#for J in [3]:
     # FILE NAME
     ncfile_current = title_val[J] + '_current.nc'
     #ncfile_chem = title_val[J] + '_chem.nc'
     fid = nc.Dataset(ncfile_current,'r')
     #fid2 = nc.Dataset(ncfile_chem,'r')

     u = fid.variables['u'[:]]
     #v = fid.variables['v'[:]]
     #temp = fid2.variables['temp'[:]]
     u = u[:]

     u_avg = np.squeeze(np.mean(u[:,:,:,:],axis=0))
     #temp_avg = np.squeeze(np.mean(temp[:,:,:,:],axis=0))

     u_slice,u_Z,u_X,u_Y = py.tools.islice(u_avg,col,VIP,'u')
     #temp_slice, temp_Z, temp_X, temp_Y = py.tools.islice(temp_avg,col,VIP,'rho')

     # DATA SUBPLOTS
     #plt.subplot(232)
     #if J > 0:
     #     plt.gca().collections.remove(C3.collections[J-1])
     #C3 = plt.contourf(X,u_Z[:,rows],u_slice[:,rows],100)   
     #plt.clim(-.3,0.3)  

     #plt.subplot(233)

     n = n+1
     plt.subplot(3,4,n)
     #if J > 0:
     #     plt.gca().collections.remove(C1.collections[J-1])
     C1 = plt.contourf(X,u_Z[:,rows],u_slice[:,rows],100) 
     plt.clim(-.3,0.3)
     #cbar1 = plt.colorbar()
     plt.title(title_val[J] + ' U velocity (m/s)')
     if J > 8:     
         plt.xlabel('J Direction')

     #plt.subplot(133)
     #if J > 0:
     #     plt.gca().collections.remove(C2.collections[J-1])
     #C2 = plt.contourf(X,temp_Z[:,rows],temp_slice[:,rows],100)
     #plt.clim(4,35)
     #cbar2 = plt.colorbar()
     #plt.title(title_val[J] + ' Temperature oC')
     #plt.xlabel('J Direction')
     #plt.pause(1)

plt.show()

