...
def plt_soi(ax):
    ...
    for nyr in range(1996,2000):
        ...
        for nmon in range(12):
            ...
            n+=1
    ax.text(date_vals[393], -1, r'$El  Ni\~{n}o$', color ='salmon', horizontalalignment='center')
    ax.text(date_vals[700], 1,r'$La  Ni\~{n}a$', color ='cornflowerblue', horizontalalignment='center') 
...
ax.set_xticks([-1*tcks[nt],0,tcks[nt]])
axs = fig.get_axes()
...
# TRANSPORT PLOT
#fidT = nc.Dataset(tport100_file)
#fidS = nc.Dataset(tport2_file)
...
#surf_pos_trans = fidS.variables['pos_transport'][:]
#surf_neg_trans = fidS.variables['neg_transport'][:]

#ax3 = ax.twinx()
#ax4 = ax.twinx()
#fSv = 1000000

#ax3.plot(plot_time,pos_trans/fSv,color='darkviolet')
#ax3.plot(plot_time,neg_trans/fSv,color='darkorange')

#ax4.plot(plot_time,surf_pos_trans/fSv,color='darkviolet')
#ax4.plot(plot_time,surf_neg_trans/fSv,color='darkorange')
...
#ax3.set_ylim(-1.6,1.3)
#ax3.plot([plot_time[0],plot_time[-1]],[0,0],'--k')

#ax4.set_yticks([-.01,0,.01])
#ax4.set_ylim(-.08,.05)
#ax4.plot([plot_time[0],plot_time[-1]],[0,0],'-k')
